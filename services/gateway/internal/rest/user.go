...
import (
	...
	user2 "github.com/what-da-flac/wtf/services/gateway/internal/domain/user"
...
func (x *Server) PostV1UserList(w http.ResponseWriter, r *http.Request) {
	...
	res, err := user2.NewList(x.repository).List(ctx, payload)
	if err != nil {
...
func (x *Server) PostV1UsersLogin(w http.ResponseWriter, r *http.Request) {
	...
	res, err := user2.NewLogin(x.identifier, x.repository, x.timer).Login(ctx, payload)
	if err != nil {
		ihandlers.WriteResponse(w, http.StatusInternalServerError, nil, err)
		return
	}
...
func (x *Server) PostV1Users(w http.ResponseWriter, r *http.Request) {
	...
	res, err := user2.NewCreate(x.identifier, x.repository, x.timer).Save(ctx, payload)
	if err != nil {
		ihandlers.WriteResponse(w, http.StatusInternalServerError, nil, err)
		return
	}
...
func (x *Server) DeleteV1UsersId(w http.ResponseWriter, r *http.Request, id string) {
	ctx := x.context(r)
	if err := user2.NewDelete(x.repository).Delete(ctx, id); err != nil {
		ihandlers.WriteResponse(w, http.StatusInternalServerError, nil, err)
		return
	}
...
func (x *Server) GetV1UsersId(w http.ResponseWriter, r *http.Request, id string) {
	ctx := x.context(r)
	res, err := user2.NewLoad(x.repository).Load(ctx, &models.User{Id: id})
	if err != nil {
...
func (x *Server) PutV1UsersId(w http.ResponseWriter, r *http.Request, id string) {
	...
	res, err := user2.NewUpdate(x.repository, x.timer).Save(ctx, id, payload)
	if err != nil {
		ihandlers.WriteResponse(w, http.StatusInternalServerError, nil, err)
		return
	}
