...
import (
	...
	listeners2 "github.com/what-da-flac/wtf/services/gateway/internal/listeners"
...
func Listen(config *environment.Config, logger interfaces2.Logger) error {
	var messageListeners []interfaces2.MessageListener
	...
	timer := timers.New()
	jobs := listeners2.NewJobs(sess, config, logger, repository, sender, timer, identifier)
	...
	for _, listener := range messageListeners {
		wg.Add(1)
		go func(l interfaces2.MessageListener) {
			defer wg.Done()
